\section{Literatura}

\begin{frame}[allowframebreaks]{Literatura}
	\begin{thebibliography}{9}
		\setbeamertemplate{bibliography item}[book]
      \bibitem{wit1}{
				Romuald Wit
				\newblock Wstęp do metod optymalizacji nieliniowej
				\newblock skrypt UJ, 1983
			}
			\bibitem{wit2}{
				Romuald Wit
				\newblock Metody programowania nieliniowego
				\newblock WNT, Biblioteka Inżynierii Oprogramowania
			}
		\setbeamertemplate{bibliography item}[online]
		\bibitem{minuit}{
			F.~Jarnes
			\newblock MINUIT -- Function Minimization and Error Analysis
			\newblock \url{http://consult.cern.ch/writeups/minuit} % dead
		}
		\bibitem{aqua}{
			Metody optymalizacji
			\newblock \url{http://aquarium.ia.agh.edu.pl/labs/opt/metopt.htm} % dead
		}
		\bibitem{smitha}{
			John~Smith
			\newblock Przykłady ciekawych wielowymiarowych funkcji
			\newblock \url{http://www.maths.adelaide.edu.au/Applied/llazausk/alife/realopt.htm} % dead
		}
		\setbeamertemplate{bibliography item}[book]
		\bibitem{rosenbrock}{
			H.~H.~Rosenbrock
			\newblock An automatic method for finding the greatest
			or least value of a function
			\newblock Comput J. 3 (1960) 175
		}
		\setbeamertemplate{bibliography item}[online]
		\bibitem{rosenbrock2}{
			Math Works Inc.
			\newblock Metoda Rosenbrocka
			\newblock \url{http://www.mathworks.com/products/optimization/example1.shtml} % dead
		}
		\setbeamertemplate{bibliography item}[book]
		\bibitem{nelder}{
			J.~A.~Nelder, R.~Mead
			\newblock A simplex method for function minimization
			\newblock Comput.~J.~7 (1965) 308
		}
		\bibitem{fletcher_reeves}{
			R. Fletcher, C. M. Reeves
			\newblock Function minimalization by conjugate gradients
			\newblock Comput. J. 7 (1964) 149
		}
		\setbeamertemplate{bibliography item}[online]
		\bibitem{flatters}{
			Chris Flatters
			\newblock The Least-Squares Fitting Tool
			\newblock \url{http://www.nrao.edu/cflatter/slime-man/node105.html} % dead
		}
		\bibitem{rupp}{
			Bernard Rupp
			\newblock The Linear Least Squares Minimization Problem
			\newblock \url{http://www.structure.llnl.gov/Xray/comp/lsq.htm} % dead
		}
		\bibitem{cernpl}{
			Biblioteki CERN-u
			\newblock CERN Program Library
			\newblock \url{http://www.cmd2.inp.nsk.su/cernlib/shortwrups/crnfinal.html} % dead
		}
		\bibitem{cullsf}{
			Constrained and Unconstrained Linear Least Squares Fitting
			\newblock \url{http://www.cmd2.inp.nsk.su/cernlib/shortwrups/node118.html} % dead
		}
    \end{thebibliography}
\end{frame}
