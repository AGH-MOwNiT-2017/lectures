...
%%%%%%%%%%%%%%%%
	...
	\begin{frame}[fragile]{Realizacja z użyciem BLAS level 1}
		...
		\end{lstlisting}
	\end{frame}
	\begin{frame}{Realizacja z użyciem BLAS level 1 I}
		\begin{tabular}{ l l l }
	...
	\textbf{A} - nxn upper or lower - triangular matrix A \\
	\textbf{uplo} - upper/lower triangular opt \\
	...
	$T - : x \leftarrow A^{-T} x;$ \\
	$C - : x \leftarrow A^{-H} x$ \\
	\textbf{diag} - whether A is unit triangular i.e. $a_{ii} = 1$ \\
	N - diag of A stored in array; \\
	U - diag = 1, \\
	\textbf{n} - no of rows and columns in A \\
	\textbf{A} - array containing n-by-n triangular matrix \\
	\textbf{lda} - the leading dimension of array a (decl. in call p) $lda \geq max(n, 1)$
	\textbf{x} - array of $lenx = (n - 1) * |incx| + 1 \leftarrow$ rhs $n$-vector $x$ \\
	\textbf{incx} - increment for x
