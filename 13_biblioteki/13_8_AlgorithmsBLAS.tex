...
%%%%%%%%%%%%%%%%
	...
	\begin{frame}[fragile]{Realizacja z użyciem BLAS level 1}
		...
		\end{lstlisting}
	\end{frame}
	\begin{frame}{Realizacja z użyciem BLAS level 1}
		FUNCTION \textunderscore DOT(N,X, INCX,Y,INCY) = dot $ \leftarrow x^T * y $ \\ % !!!!!!!!!!!! Not finished part
	...
	\textunderscore TRSV(UPLO, TRANS, DIAG, N, A, LDA, X, INCX) \\ % !!!!!!!!!!!! Not finished part
	we: $ Ay = x $, wy: $x \leftarrow y$ \\ % !!!!!!!!!!!! Not finished part, no space on slide
	...
	\end{frame}
	\begin{frame}[fragile]{Realizacja z użyciem BLAS level 2 III}
		Program ma postać:
		\begin{lstlisting}
		DO 10 J=1, N
	...
	$$
		U_{11}^T * U_{12} = A_{12} %TRSM
	$$
	$$
		U_{22}^T * U_{22} = A_{22} - U_{12}^T * U_{12} %SYRK
	$$
	\end{frame}
	...
	\begin{frame}{Realizacja z użyciem BLAS level 3}
		...
		B $\leftarrow \alpha$ op $(A^{-1})*B$; B $\leftarrow \alpha$ * B op ($A^{-1}$) %!!!!!!!!!! this line and next two
	...
	\begin{frame}{Realizacja z użyciem BLAS level 3}
		...
		rank k update to o real symmetric, complex symmetric, complex Herm. % Misspelling?
		$
		\begin{cases}
	...
	\begin{frame}[fragile]{Realizacja z użyciem BLAS level 3}
	Program ma postać
		\begin{lstlisting}
			DO 10 J=1, N, NB
