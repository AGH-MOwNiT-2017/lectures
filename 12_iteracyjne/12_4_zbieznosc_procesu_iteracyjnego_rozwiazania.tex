...
\begin{frame}{Zbieżność procesu iteracyjnego rozwiązywania Ax=b}
  ...
  \begin{block}{Teza:}
    \center Ciąg ($\star$) z dowolnym wektorem startowym $x^{(0)}$ jest zbieżny do jedynego granicznego $x^{(\inf)}$ wtedy i tylko wtedy, gdy \emph{promień spektralny (sectral radius)} macierzy iteracji
    \center $\rho(M)<1$
  \end{block}
\end{frame}
