...
\begin{frame}{}
  \begin{block}{Dowód}
    ...
    $$ \frac{||\varepsilon^{(t)}||}{||\varepsilon^{(0)}||} \leq ||M^{(t)}|| \text{ - zadana dokładność } \rho\Rightarrow ||M^{(t)}|| = 10^{-p}$$   %TODO - sprawdź oznaczenia!
    \\$\lambda$ -- \emph{asymptotic convergence factor}
    \[\lambda^{t}=10^{-p} \rightarrow t = -p \cdot  \frac{ln 10}{ln \lambda} \text{ - liczba iteracji}\]    %TO CHECK - there was a star near t
  \end{block}
\end{frame}
